...
Dim_Tiempo(id_tiempo, fecha, año, mes, dia, trimestre, nombre_mes, dia_semana)

Dim_Clientes(id_cliente, nombre, apellido, cedula)

Dim_Productos(id_producto, nombre_producto, categoria, precio_unitario, stock)

Hecho_Ventas(id_venta, id_cliente, id_producto, id_tiempo, fecha_venta, cantidad, precio_detalle, Total)

Relaciones (JOINs permitidos):

Hecho_Ventas.id_cliente → Dim_Clientes.id_cliente

Hecho_Ventas.id_producto → Dim_Productos.id_producto

Hecho_Ventas.id_tiempo → Dim_Tiempo.id_tiempo

⚠️ Reglas obligatorias:
Solo se permiten consultas SQL de tipo SELECT.

NO generar consultas INSERT, UPDATE, DELETE, DROP, ALTER, TRUNCATE, ni ningún otro tipo.

Usar únicamente las columnas listadas en las tablas.

Las consultas deben estar en una sola línea, sin saltos de línea, sin comillas triples ni formato adicional.

Las consultas deben ser seguras, limpias y eficientes.

Si se solicita agrupación, usar GROUP BY con funciones agregadas como SUM, COUNT, AVG, etc.

Si el usuario no lo pide, no incluir cláusulas WHERE, ORDER BY, LIMIT, etc.

Utilizar solo los JOINs definidos en las relaciones.
...
.${mensaje}"
    `;
            ...
            const response = await fetch('http://localhost:3000/api/consultarconia', {
                method: 'POST',
